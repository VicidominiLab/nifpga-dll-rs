...
use fehler::throws;
use nifpga_dll_type_macro::datatype;
...
impl Type for f32 {
    ...
    fn read(session: u32, indicator: u32) -> Self {
        ...
        unsafe {
            nifpga_dll_sys::read_f32(session, indicator, &mut value).to_result()?;
        };
        value
    }
    ...
    fn write(session: u32, control: u32, value: Self) {
        unsafe {
            nifpga_dll_sys::write_f32(session, control, value).to_result()?;
        }
    }
    ...
    fn read_array(session: u32, indicator: u32, slice: &mut [Self]) {
        unsafe {
            nifpga_dll_sys::read_array_f32(session, indicator, slice.as_mut_ptr(), slice.len())
                .to_result()?;
        }
    }
    ...
    fn write_array(session: u32, indicator: u32, slice: &[Self]) {
        unsafe {
            nifpga_dll_sys::write_array_f32(session, indicator, slice.as_ptr(), slice.len())
                .to_result()?;
        }
    }
    ...
    unsafe fn acquire_fifo_read_elements(
        ...
        nifpga_dll_sys::acquire_fifo_read_elements_f32(
            session,
            fifo,
            &mut ptr,
            num_elements,
    ...
    unsafe fn acquire_fifo_write_elements(
        ...
        nifpga_dll_sys::acquire_fifo_write_elements_f32(
            session,
            fifo,
            &mut ptr,
            num_elements,
