...
//     extern "C" fn cleanup() {
//         unsafe {
//             nifpga_dll_sys::finalize().to_result().unwrap();
//         }
//     }
//
...
//     unsafe {
//         INIT.call_once(|| {
//             STATUS = nifpga_dll_sys::initialize();
...
impl Session {
    ...
    #[throws(NifpgaError)]
    pub fn run(&self) {
        unsafe { nifpga_dll_sys::run(self.handle).to_result()? }
    }

    #[throws(NifpgaError)]
    pub fn abort(&self) {
        unsafe { nifpga_dll_sys::abort(self.handle).to_result()? }
    }

    #[throws(NifpgaError)]
    pub fn reset(&self) {
        unsafe { nifpga_dll_sys::reset(self.handle).to_result()? }
    }

    #[throws(NifpgaError)]
    pub fn download(&self) {
        unsafe { nifpga_dll_sys::download(self.handle).to_result()? }
    }

    #[throws(NifpgaError)]
    pub fn acknowledge_irqs(&self, irqs: u32) {
        unsafe { nifpga_dll_sys::acknowledge_irqs(self.handle, irqs).to_result()? }
    }
...
impl Drop for Session {
    fn drop(&mut self) {
        unsafe {
            nifpga_dll_sys::close(self.handle, if self.reset_on_close { 0 } else { 1 })
                .to_result()
                .unwrap()
        };
    }
